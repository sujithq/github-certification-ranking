...
from credly_badges import (
    ...
    get_source_urls,
    get_tracked_certification_status,
    resolve_user_id,
)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: ./fetch_user.py <user_uuid|username|profile_url> [--verbose]")
        print("Example: ./fetch_user.py 123e4567-e89b-12d3-a456-426614174000")
        print("         ./fetch_user.py john-doe --verbose")
    ...
    if verbose:
        print("[DEBUG MODE: Showing ALL unfiltered Credly badges]")
    print(f"Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 80)

    try:
        user_id = resolve_user_id(identifier)
        details = fetch_user_github_badge_details(identifier, verbose=verbose)
        tracked = get_tracked_certification_status(details)
        sources = get_source_urls(user_id)
        ...
        output = {
            ...
            "counts": {
                ...
                "tracked_total": tracked["tracked_total"],
                "tracked_completed_active": tracked["completed_active"],
                "tracked_completed_expired": tracked["completed_expired"],
                "tracked_missing": tracked["status_counts"]["missing"],
                "tracked_expired": tracked["status_counts"]["expired"],
                "tracked_active": tracked["status_counts"]["active"],
                "tracked_other": tracked["status_counts"]["other"],
                "tracked_unknown": tracked["status_counts"]["unknown"],
            },
            "badges": {
            ...
            "tracked_certifications": {
                "status_counts": tracked["status_counts"],
        ...
        if verbose and details["external"].get("_debug_all_microsoft_badges"):
            output["_debug_external_microsoft_badges"] = details["external"]["_debug_all_microsoft_badges"]
        if verbose and details["external"].get("_debug_all_external_badges"):
            output["_debug_all_external_badges"] = details["external"]["_debug_all_external_badges"]
        if verbose and details["org"].get("_debug_all_org_badges"):
            output["_debug_all_org_badges"] = details["org"]["_debug_all_org_badges"]
